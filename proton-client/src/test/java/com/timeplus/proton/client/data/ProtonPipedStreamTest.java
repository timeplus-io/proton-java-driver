...
public class ProtonPipedStreamTest {
    @Test(groups = { "unit" })
    public void testRead() throws Exception {
        ProtonPipedStream stream = new ProtonPipedStream(4, 3, 1);
        ...
        stream.queue.clear();
        stream.queue.put(ProtonByteBuffer.EMPTY_BUFFER);
        Assert.assertEquals(stream.queue.size(), 1);
        try (InputStream in = stream.getInput()) {
    ...
    public void testWrite() throws Exception {
        ProtonPipedStream stream = new ProtonPipedStream(2, 3, 2);
        Assert.assertEquals(stream.queue.size(), 0);
        ...
        stream = new ProtonPipedStream(1, 1, 2);
        Assert.assertEquals(stream.queue.size(), 0);
        try (OutputStream out = stream) {
            out.write(5);
            Assert.assertEquals(stream.queue.size(), 0);
    ...
    public void testPipedStream() throws Exception {
        ...
        for (int bufferSize = -1; bufferSize < 10; bufferSize++) {
            for (int queueLength = -1; queueLength < 10; queueLength++) {
                ProtonPipedStream stream = new ProtonPipedStream(bufferSize, queueLength, 10000);
