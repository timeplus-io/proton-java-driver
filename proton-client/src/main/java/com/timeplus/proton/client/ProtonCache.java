...
import com.timeplus.proton.client.cache.CaffeineCache;
import com.timeplus.proton.client.cache.JdkLruCache;

/**
 * Wrapper interface depicts essential methods required by a client-side cache.
 */
public interface ProtonCache<K, V> {
    /**
     * Default cache size.
     */
     ...
     * @return cache
     */
    static <K, V> ProtonCache<K, V> create(int capacity, long expireSeconds, Function<K, V> loadFunc) {
        ProtonCache<K, V> cache;
