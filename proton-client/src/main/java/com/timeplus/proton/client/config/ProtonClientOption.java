...
import com.timeplus.proton.client.ProtonCompression;
import com.timeplus.proton.client.ProtonFormat;
import com.timeplus.proton.client.*;

/**
 * Generic client options.
 */
public enum ProtonClientOption implements ProtonOption {
    /**
     * Whether the client should run in async mode(e.g.
     * {@link ProtonClient#execute(ProtonRequest)}
     ...
     */
    COMPRESS_ALGORITHM("compress_alogrithm", ProtonCompression.LZ4,
     ...
     */
    DECOMPRESS_ALGORITHM("decompress_alogrithm", ProtonCompression.LZ4,
     ...
     * Default format.
     */
    FORMAT("format", ProtonFormat.TabSeparated, "Default format."),
    /**
    ...
    /**
     * Whether to reuse wrapper of value(e.g. ProtonValue or
     * ProtonRecord) for memory efficiency.
     */
    REUSE_VALUE_WRAPPER("reuse_value_wrapper", true,
            "Whether to reuse wrapper of value(e.g. ProtonValue or ProtonRecord) for memory efficiency."),
    /**
     * Server timezone.
     */
     ...
     * SSL mode.
     */
    SSL_MODE("sslmode", ProtonSslMode.STRICT,
    ...
    private static final Map<String, ProtonClientOption> options;

    static {
        Map<String, ProtonClientOption> map = new HashMap<>();

        for (ProtonClientOption o : values()) {
     ...
     */
    public static ProtonClientOption fromKey(String key) {
     ...
     */
    <T extends Serializable> ProtonClientOption(String key, T defaultValue, String description) {
