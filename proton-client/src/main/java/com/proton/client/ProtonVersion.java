...
/**
 * Immutable Proton version, which takes the form
 ...
 * comparison.
 */
public final class ProtonVersion implements Comparable<ProtonVersion>, Serializable {
    private static final long serialVersionUID = 6721014333437055314L;

    private static final String STR_LATEST = "latest";

    private static final ProtonVersion defaultVersion = new ProtonVersion(false, 0, 0, 0, 0);
    private static final ProtonVersion latestVersion = new ProtonVersion(true, 0, 0, 0, 0);

    private static final ProtonCache<String, ProtonVersion> versionCache = ProtonCache.create(100, 300,
            ProtonVersion::parseVersion);
    ...
    public static boolean check(String version, String range) {
        return ProtonVersion.of(version).check(range);
    }
    ...
    public static int compare(String fromVersion, String toVersion) {
        return ProtonVersion.of(fromVersion).compareTo(ProtonVersion.of(toVersion));
    }
     ...
     */
    public static ProtonVersion of(String version) {
     ...
     * @return version
     */
    public static ProtonVersion of(int yearOrMajor, int... more) {
        int len = more != null ? more.length : 0;
        return new ProtonVersion(false, yearOrMajor, len > 0 ? more[0] : 0, len > 1 ? more[1] : 0,
                len > 2 ? more[2] : 0);
    }
     ...
     * @return parsed version
     */
    protected static ProtonVersion parseVersion(String version) {
        if (version == null || version.isEmpty()) {
            return defaultVersion;
        }
        ...
        return latest ? latestVersion : new ProtonVersion(false, parts[0], parts[1], parts[2], parts[3]);
    }
    ...
    protected ProtonVersion(boolean latest, int year, int feature, int maintenance, int build) {
        this.latest = latest;
     ...
     */
    protected int compareTo(ProtonVersion o, boolean sameSeriesComparison) {
        if (o == null) {
            o = defaultVersion;
        }
     ...
     */
    public boolean belongsTo(ProtonVersion version) {
        if (version == null) {
    ...
    public boolean belongsTo(String version) {
        return belongsTo(ProtonVersion.of(version));
    }
     ...
     * when comparing "21.3.1.1" to "21.3", {@link #compareTo(ProtonVersion)}
     ...
     *         otherwise
     */
    public boolean isNewerOrEqualTo(ProtonVersion version) {
    ...
    public boolean isNewerOrEqualTo(String version) {
        return isNewerOrEqualTo(ProtonVersion.of(version));
    }
     ...
     */
    public boolean isNewerThan(ProtonVersion version) {
    ...
    public boolean isNewerThan(String version) {
        return isNewerThan(ProtonVersion.of(version));
    }
     ...
     *         otherwise
     */
    public boolean isOlderOrEqualTo(ProtonVersion version) {
        return compareTo(version, true) <= 0;
    }
    ...
    public boolean isOlderOrEqualTo(String version) {
        return isOlderOrEqualTo(ProtonVersion.of(version));
    }
     ...
     */
    public boolean isOlderThan(ProtonVersion version) {
        return compareTo(version, false) < 0;
    }
    ...
    public boolean isOlderThan(String version) {
        return isOlderThan(ProtonVersion.of(version));
    }
    ...
    public boolean check(String range) {
        ...
        for (int i = 0, len = range.length(); i < len; i++) {
            ...
            if ((ch >= '0' && ch <= '9') || ch == '.') {
                ...
                ProtonVersion v1 = null;
                ProtonVersion v2 = null;
                ...
                for (i = i + 1; i < len; i++) {
                    ...
                    if ((nextCh >= '0' && nextCh <= '9') || nextCh == '.') {
                        ...
                        v1 = ProtonVersion.of(builder.toString());
                        builder.setLength(0);
                    } else if (nextCh == ')' || nextCh == ']') {
                        if (builder.length() == 0) {
                            v2 = latestVersion;
                        } else {
                            v2 = ProtonVersion.of(builder.toString());
                            builder.setLength(0);
                        }
                        break;
                    }
                ...
                ProtonVersion v = of(builder.toString());
                builder.setLength(0);
    ...
    @Override
    public int compareTo(ProtonVersion o) {
    ...
    public boolean equals(Object obj) {
        ...
        ProtonVersion other = (ProtonVersion) obj;
