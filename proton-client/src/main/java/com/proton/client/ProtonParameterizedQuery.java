 ...
 */
public class ProtonParameterizedQuery implements Serializable {
    ...
    protected static class QueryPart implements Serializable {
        ...
        public final ProtonColumn paramType;

        protected QueryPart(ProtonConfig config, String part, int paramIndex, String paramName, String paramType,
                Map<String, ProtonValue> map) {
            this.part = part;
            this.paramIndex = paramIndex;
            this.paramName = paramName != null ? paramName : String.valueOf(paramIndex);
            if (paramType != null) {
                this.paramType = ProtonColumn.of("", paramType);
                map.put(paramName, ProtonValues.newValue(config, this.paramType));
            } else {
                this.paramType = null;
    ...
    public static void apply(StringBuilder builder, String sql, Map<String, String> params) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (ProtonUtils.isQuote(ch)) {
                int endIndex = ProtonUtils.skipQuotedString(sql, i, len, ch);
                ...
                if (ch == '-' && nextCh == ch) {
                    endIndex = ProtonUtils.skipSingleLineComment(sql, i + 2, len);
                    builder.append(sql.substring(i, endIndex));
                    i = endIndex - 1;
                } else if (ch == '/' && nextCh == '*') {
                    endIndex = ProtonUtils.skipMultiLineComment(sql, i + 2, len);
                    builder.append(sql.substring(i, endIndex));
                    i = endIndex - 1;
                } else if (ch == ':') {
                    if (nextCh == ch) { // skip PostgreSQL-like type conversion
                        ...
                        for (i = i + 2; i < len; i++) {
                            char c = sql.charAt(i);
                            if (c == '(') {
                                i = ProtonUtils.skipBrackets(sql, i, len, c) - 1;
                                break;
                        ...
                        builder.append(params.getOrDefault(sb.toString(), ProtonValues.NULL_EXPR));
                    } else {
                        builder.append(ch);
                    }
                } else {
     ...
     */
    public static ProtonParameterizedQuery of(ProtonConfig config, String query) {
        // cache if query.length() is greater than 1024?
        return new ProtonParameterizedQuery(config, query);
    }
    ...
    private final Map<String, ProtonValue> names;
     ...
     */
    protected ProtonParameterizedQuery(ProtonConfig config, String query) {
        this.config = ProtonChecker.nonNull(config, "config");
        originalQuery = ProtonChecker.nonBlank(query, "query");
                    ...
                    if (nextCh == ch) { // skip PostgreSQL-like type conversion
                        ...
                        for (i = i + 2; i < len; i++) {
                            ...
                            if (Character.isJavaIdentifierPart(c)) {
                                ...
                                if (c == '(') {
                                    int idx = ProtonUtils.skipBrackets(originalQuery, i, len, c);
    ...
    protected String toSqlExpression(String paramName, Object value) {
        ProtonValue template = names.get(paramName);
        return template != null ? template.update(value).toSqlExpression()
                : ProtonValues.convertToSqlExpression(value);
    }
    ...
    public void apply(StringBuilder builder, Map<String, String> params) {
        ...
        for (QueryPart p : parts) {
            ...
            builder.append(params.getOrDefault(p.paramName, ProtonValues.NULL_EXPR));
        }
     ...
     * {@link ProtonValues#convertToSqlExpression(Object)} will be used to
    ...
    public void apply(StringBuilder builder, Object param, Object... more) {
        ...
        int index = -1;
        for (Entry<String, ProtonValue> e : names.entrySet()) {
            ProtonValue v = e.getValue();
            if (index < 0) {
                map.put(e.getKey(),
                        v != null ? v.update(param).toSqlExpression() : ProtonValues.convertToSqlExpression(param));
            } else if (index < len) {
                map.put(e.getKey(), v != null ? v.update(more[index]).toSqlExpression()
                        : ProtonValues.convertToSqlExpression(more[index]));
            } else {
                break;
            }
            index++;
        ...
        for (Entry<String, ProtonValue> e : names.entrySet()) {
            ...
            if (index < len) {
                map.put(e.getKey(), v != null ? v.update(values[index]).toSqlExpression()
                        : ProtonValues.convertToSqlExpression(values[index]));
            } else {
                break;
            }
            index++;
     ...
     */
    public ProtonValue[] getParameterTemplates() {
        int i = 0;
        ProtonValue[] tempaltes = new ProtonValue[names.size()];
        for (ProtonValue v : names.values()) {
            tempaltes[i++] = v;
        }
        return tempaltes;
    }
    ...
    public boolean equals(Object obj) {
        ...
        ProtonParameterizedQuery other = (ProtonParameterizedQuery) obj;
