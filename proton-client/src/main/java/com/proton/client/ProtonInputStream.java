 ...
 */
public abstract class ProtonInputStream extends InputStream {
    /**
     * Empty byte array.
     */
    @Deprecated
    public static final byte[] EMPTY_BYTES = ProtonByteBuffer.EMPTY_BYTES;
    /**
     * Empty and read-only byte buffer.
     */
    @Deprecated
    public static final ByteBuffer EMPTY_BUFFER = ProtonByteBuffer.EMPTY_BUFFER;
    ...
    static final class BlockingInputStream extends ProtonInputStream {
        ...
        BlockingInputStream(BlockingQueue<ByteBuffer> queue, int timeout, Runnable afterClose) {
            ...
            this.queue = ProtonChecker.nonNull(queue, "Queue");
        ...
        private void ensureOpen() throws IOException {
            if (closed) {
                throw new IOException(
                        ProtonUtils.format("Blocking input stream(queue: %d, buffer: %d) has been closed",
                                queue.size(), buffer != null ? buffer.remaining() : 0));
            }

            if (buffer == null || (buffer != ProtonByteBuffer.EMPTY_BUFFER && !buffer.hasRemaining())) {
                updateBuffer();
            }
        }
        ...
        private int updateBuffer() throws IOException {
            try {
                if (timeout > 0) {
                    ...
                    if (buffer == null) {
                        throw new IOException(ProtonUtils.format("Read timed out after %d ms", timeout));
                    }
                } else {
                    buffer = queue.take();
                }
        ...
        public int available() throws IOException {
            if (closed || buffer == ProtonByteBuffer.EMPTY_BUFFER) {
                return 0;
            }
        ...
        public int peek() throws IOException {
            ...
            if (buffer == ProtonByteBuffer.EMPTY_BUFFER) {
                return -1;
            }
        ...
        public long pipe(ProtonOutputStream output) throws IOException {
            ...
            while (buffer != ProtonByteBuffer.EMPTY_BUFFER) {
            ...
            while (len > 0) {
                if (buffer == ProtonByteBuffer.EMPTY_BUFFER) {
                    ...
                    throw offset == 0 ? new EOFException()
                            : new IOException(ProtonUtils.format(INCOMPLETE_READ_ERROR, offset, length));
                }
        ...
        public long skip(long n) throws IOException {
            ...
            if (n == Long.MAX_VALUE) {
                ...
                while (buffer != ProtonByteBuffer.EMPTY_BUFFER && buffer.limit() > 0) {
    ...
    static final class WrappedInputStream extends ProtonInputStream {
        ...
        WrappedInputStream(InputStream input, int bufferSize, Runnable afterClose) {
            ...
            this.in = ProtonChecker.nonNull(input, "InputStream");
            this.buffer = new byte[ProtonChecker.between(bufferSize, "BufferSize", MIN_BUFFER_SIZE,
                    MAX_BUFFER_SIZE)];
        ...
        private void ensureOpen() throws IOException {
            if (closed) {
                throw new IOException(ProtonUtils.format("Wrapped input stream(%s) has been closed", in));
            }
        }
        ...
        public int read(byte[] b, int off, int len) throws IOException {
            if ((len | off | b.length) < 0 || len > b.length - off) {
                ...
                // in case b is the byte array return from ProtonByteBuffer.array()
        ...
        public byte[] readBytes(int length) throws IOException {
            if (length < 1) {
                ...
                if (!updateBuffer()) {
                    closeQuietly();
                    throw new EOFException(
                            ProtonUtils.format("Failed to read %d bytes due to end of stream", length));
                } else if (length > limit) {
                    throw new EOFException(ProtonUtils.format("Reached end of stream after reading %d bytes of %d",
                            limit, length));
                }
                byte[] bytes = new byte[length];
                System.arraycopy(buffer, position, bytes, 0, length);
            ...
            while (counter < length) {
                if (limit > position || updateBuffer()) {
                    ...
                    throw counter == 0 ? new EOFException()
                            : new IOException(ProtonUtils.format(INCOMPLETE_READ_ERROR, counter, bytes.length));
                }
            }
            return bytes;
        }
     ...
     */
    public static ProtonInputStream of(BlockingQueue<ByteBuffer> queue, int timeout) {
     ...
     * @return wrapped input
     */
    public static ProtonInputStream of(BlockingQueue<ByteBuffer> queue, int timeout, Runnable afterClose) {
     ...
     *         {@link ProtonInputStream}
     */
    public static ProtonInputStream of(InputStream input) {
        return of(input, (int) ProtonClientOption.MAX_BUFFER_SIZE.getDefaultValue(), null);
    }
     ...
     *         {@link ProtonInputStream}
     */
    public static ProtonInputStream of(InputStream input, int bufferSize) {
     ...
     *         {@link ProtonInputStream}
     */
    public static ProtonInputStream of(InputStream input, int bufferSize, Runnable afterClose) {
        return input instanceof ProtonInputStream ? (ProtonInputStream) input
                : new WrappedInputStream(input, bufferSize, afterClose);
    }

    protected final Runnable afterClose;
    protected final ProtonByteBuffer byteBuffer;

    protected boolean closed;

    protected ProtonInputStream(Runnable afterClose) {
        this.afterClose = afterClose;
        this.byteBuffer = ProtonByteBuffer.newInstance();
     ...
     *                     end of the stream
     */
    public abstract long pipe(ProtonOutputStream output) throws IOException;
    ...
    public String readString(int byteLength, Charset charset) throws IOException {
        ...
        ProtonByteBuffer buf = read(byteLength);
