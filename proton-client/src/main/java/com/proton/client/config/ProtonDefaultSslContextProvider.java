...
public class ProtonDefaultSslContextProvider implements ProtonSslContextProvider {
    /**
    ...
    protected KeyStore getKeyStore(String sslRootCert)
        ...
        try {
            ...
            throw new NoSuchAlgorithmException(
                    ProtonUtils.format("%s KeyStore not available", KeyStore.getDefaultType()));
        }

        try (InputStream in = ProtonUtils.getFileInputStream(sslRootCert)) {
    ...
    protected SSLContext getJavaSslContext(ProtonConfig config) throws SSLException {
        ...
        try {
            ...
            if (sslMode == ProtonSslMode.NONE) {
                ...
                throw new IllegalArgumentException(ProtonUtils.format("unspported ssl mode '%s'", sslMode));
            }
    ...
    @Override
    public <T> Optional<T> getSslContext(Class<? extends T> sslContextClass, ProtonConfig config)
            throws SSLException {
