...
import com.timeplus.proton.jdbc.parser.ProtonSqlParser;
...
public class JdbcParseHandlerTest {
    ...
    public void testParseDeleteStatement() {
        Assert.assertEquals(
                ProtonSqlParser.parse("delete from tbl", null, JdbcParseHandler.INSTANCE)[0]
                        .getSQL(),
                "TRUNCATE TABLE tbl");
        Assert.assertEquals(
                ProtonSqlParser.parse("delete from tbl where 1", null, JdbcParseHandler.INSTANCE)[0]
                        .getSQL(),
                "ALTER TABLE `tbl` DELETE where 1 SETTINGS mutations_sync=1");
        Assert.assertEquals(
                ProtonSqlParser.parse("delete from tbl where 1 and 1 settings a=1 format CSV", null,
                        JdbcParseHandler.INSTANCE)[0].getSQL(),
                "ALTER TABLE `tbl` DELETE where 1 and 1 SETTINGS mutations_sync=1, a=1 format CSV");
        Assert.assertEquals(
                ProtonSqlParser.parse(
    ...
    public void testParseUpdateStatement() {
        Assert.assertEquals(
                ProtonSqlParser.parse("update tbl set a=1", null, JdbcParseHandler.INSTANCE)[0]
                        .getSQL(),
                "ALTER TABLE `tbl` UPDATE a=1 SETTINGS mutations_sync=1");
        Assert.assertEquals(
                ProtonSqlParser.parse("update tbl set a=1,b=2 where 1", null,
    ...
    public void testDeleteStatementWithoutWhereClause() {
        Assert.assertEquals(JdbcParseHandler.INSTANCE.handleStatement("delete  from  `a\\`' a` . tbl",
                ...
                new ProtonSqlStatement("TRUNCATE TABLE  `a\\`' a` . tbl", StatementType.DELETE,
                        null,
        ...
        Assert.assertEquals(JdbcParseHandler.INSTANCE.handleStatement("delete from  `table\\`'1`",
                ...
                new ProtonSqlStatement("TRUNCATE TABLE  `table\\`'1`", StatementType.DELETE, null,
                        null,
    ...
    public void testDeleteStatementWithSettings() {
        ...
        Assert.assertEquals(
                ...
                new ProtonSqlStatement("TRUNCATE TABLE tbl settings a=1", StatementType.DELETE,
                        null, null, "tbl",
    ...
    public void testUpdateStatementWithoutWhereClause() {
        ...
        Assert.assertEquals(JdbcParseHandler.INSTANCE.handleStatement("update  `table\\`'1` set a=1",
                ...
                new ProtonSqlStatement("ALTER TABLE `table1` UPDATE a=1 SETTINGS mutations_sync=1",
                        StatementType.UPDATE, null, null, "table1", null, null, null, null,
                        null, null, null));
    }
