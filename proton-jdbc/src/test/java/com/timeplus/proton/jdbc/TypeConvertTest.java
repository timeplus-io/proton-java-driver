...
public class TypeConvertTest {
    ...
    void DateConvert() throws SQLException {
        Calendar calender = Calendar.getInstance(TimeZone.getTimeZone(ZoneId.of("UTC")));
        Assert.assertThrows(() -> {
            List<TypeTest.Pair<String, String>> struct = List.of(
            ...
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(insertPreparedSql(STREAMNAME, struct))) {
                ps.setDate(1, Date.valueOf(LocalDate.of(2149, 6, 7)), calender);
                ps.addBatch();
                ps.executeBatch();
            }
        });
        Assert.assertThrows(() -> {
            ...
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(insertPreparedSql(STREAMNAME, struct))) {
                ps.setTimestamp(2, Timestamp.valueOf(LocalDateTime.of(2106, 2, 7, 6, 28, 16)), calender);
                ps.addBatch();
                ps.executeBatch();
            }
        });
        {
            ...
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(insertPreparedSql(STREAMNAME, struct))) {
                ps.setTimestamp(1, Timestamp.valueOf(LocalDateTime.of(2023, 10, 18, 9, 59, 10, 123 * 1000 * 1000)), calender);
                ps.addBatch();
                ps.executeBatch();
            }
            wait(2);
