...
import com.timeplus.proton.except.ProtonException;
import com.timeplus.proton.except.ProtonExceptionSpecifier;
import com.timeplus.proton.response.ProtonLZ4Stream;
import com.timeplus.proton.response.ProtonResponse;
import com.timeplus.proton.response.ProtonResponseGsonDeserializer;
import com.timeplus.proton.response.ProtonResponseSummary;
import com.timeplus.proton.response.ProtonResultSet;
import com.timeplus.proton.response.ProtonScrollableResultSet;
import com.timeplus.proton.response.FastByteArrayOutputStream;
import com.timeplus.proton.settings.ProtonProperties;
import com.timeplus.proton.settings.ProtonQueryParam;
import com.timeplus.proton.util.ProtonHttpClientBuilder;
...
public class ProtonStatementImpl extends ConfigurableApi<ProtonStatement> implements ProtonStatement {

    private static final Logger log = LoggerFactory.getLogger(ProtonStatementImpl.class);
    private static final Gson gson = new GsonBuilder().registerTypeAdapter(ProtonResponse.class,
            new ProtonResponseGsonDeserializer()).create();
    ...
    protected ProtonProperties properties;

    private ProtonConnection connection;

    private ProtonResultSet currentResult;

    private ProtonRowBinaryInputStream currentRowBinaryResult;

    private ProtonResponseSummary currentSummary;
    ...
    protected List<ProtonSqlStatement> batchStmts;
    ...
    protected ProtonSqlStatement[] parseSqlStatements(String sql) throws SQLException {
        parsedStmts = ProtonSqlParser.parse(sql, null);
    ...
    protected ProtonSqlStatement parseSqlStatements(String sql, ProtonFormat preferredFormat,
        ...
        if (additionalDBParams != null && parsedStmts.length > 1 && properties.getSessionId() == null) {
            additionalDBParams.put(ProtonQueryParam.SESSION_ID, UUID.randomUUID().toString());
        }

        ProtonSqlStatement lastStmt = getLastStatement();
        ProtonSqlStatement formattedStmt = applyFormat(lastStmt, preferredFormat);
    ...
    protected ProtonSqlStatement applyFormat(ProtonSqlStatement stmt, ProtonFormat preferredFormat) {
        if (Objects.requireNonNull(stmt).isQuery() && !stmt.hasFormat()) {
            ...
            positions.put(ProtonSqlStatement.KEYWORD_FORMAT, sql.length());

            sql = new StringBuilder(sql).append("\nFORMAT ").append(format).toString();
            stmt = new ProtonSqlStatement(sql, stmt.getStatementType(), stmt.getCluster(), stmt.getDatabase(),
    ...
    protected Map<ProtonQueryParam, String> importAdditionalDBParameters(
            Map<ProtonQueryParam, String> additionalDBParams) {
        if (additionalDBParams == null || additionalDBParams.isEmpty()) {
            additionalDBParams = new EnumMap<>(ProtonQueryParam.class);
    ...
    protected ResultSet updateResult(ProtonSqlStatement stmt, InputStream is)
            throws IOException, ProtonException {
        ResultSet rs = null;
        if (stmt.isQuery()) {
            currentUpdateCount = -1;
            currentResult = createResultSet(properties.isCompress() ? new ProtonLZ4Stream(is) : is,
    ...
    protected int executeStatement(ProtonSqlStatement stmt, Map<ProtonQueryParam, String> additionalDBParams,
            List<ProtonExternalData> externalData, Map<String, String> additionalRequestParams)
            throws SQLException {
        additionalDBParams = importAdditionalDBParameters(additionalDBParams);
        stmt = applyFormat(stmt, ProtonFormat.TabSeparatedWithNamesAndTypes);
    ...
    protected ResultSet executeQueryStatement(ProtonSqlStatement stmt,
            Map<ProtonQueryParam, String> additionalDBParams, List<ProtonExternalData> externalData,
        ...
        try {
            ...
            throw ProtonExceptionSpecifier.specify(e, properties.getHost(), properties.getPort());
        }
    }

    protected ProtonResponse executeQueryProtonResponse(ProtonSqlStatement stmt,
            Map<ProtonQueryParam, String> additionalDBParams, Map<String, String> additionalRequestParams)
            throws SQLException {
        additionalDBParams = importAdditionalDBParameters(additionalDBParams);
        stmt = applyFormat(stmt, ProtonFormat.JSONCompact);

        try (InputStream is = getInputStream(stmt, additionalDBParams, null, additionalRequestParams)) {
            return readJsonResponse(properties.isCompress() ? new ProtonLZ4Stream(is) : is,
                    ProtonResponse.class);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    public ProtonStatementImpl(CloseableHttpClient client, ProtonConnection connection,
            ProtonProperties properties, int resultSetType) {
        super(null);
        this.client = client;
        this.httpContext = ProtonHttpClientBuilder.createClientContext(properties);
        this.connection = connection;
        this.properties = properties == null ? new ProtonProperties() : properties;
    ...
    @Override
    public ResultSet executeQuery(String sql, Map<ProtonQueryParam, String> additionalDBParams)
            throws SQLException {
    ...
    @Override
    public ResultSet executeQuery(String sql, Map<ProtonQueryParam, String> additionalDBParams,
            List<ProtonExternalData> externalData) throws SQLException {
    ...
    public ResultSet executeQuery(String sql, Map<ProtonQueryParam, String> additionalDBParams,
        ...
        additionalDBParams.put(ProtonQueryParam.EXTREMES, "0");

        parseSqlStatements(sql, ProtonFormat.TabSeparatedWithNamesAndTypes, additionalDBParams);

        InputStream is = getLastInputStream(additionalDBParams, externalData, additionalRequestParams);
        ProtonSqlStatement parsedStmt = getLastStatement();
    ...
    @Override
    public ProtonResponse executeQueryProtonResponse(String sql) throws SQLException {
        return executeQueryProtonResponse(sql, null);
    }

    @Override
    public ProtonResponse executeQueryProtonResponse(String sql,
            Map<ProtonQueryParam, String> additionalDBParams) throws SQLException {
        return executeQueryProtonResponse(sql, additionalDBParams, null);
    }
    ...
    public ProtonResponse executeQueryProtonResponse(String sql,
            ...
            throws SQLException {
        additionalDBParams = importAdditionalDBParameters(additionalDBParams);
        parseSqlStatements(sql, ProtonFormat.JSONCompact, additionalDBParams);
    ...
    @Override
    public ProtonRowBinaryInputStream executeQueryProtonRowBinaryStream(String sql) throws SQLException {
        return executeQueryProtonRowBinaryStream(sql, null);
    }

    @Override
    public ProtonRowBinaryInputStream executeQueryProtonRowBinaryStream(String sql,
            Map<ProtonQueryParam, String> additionalDBParams) throws SQLException {
        return executeQueryProtonRowBinaryStream(sql, additionalDBParams, null);
    }
    ...
    public ProtonRowBinaryInputStream executeQueryProtonRowBinaryStream(String sql,
            ...
            throws SQLException {
        additionalDBParams = importAdditionalDBParameters(additionalDBParams);
        parseSqlStatements(sql, ProtonFormat.RowBinaryWithNamesAndTypes, additionalDBParams);
        ...
        try {
            if (parsedStmt.isQuery()) {
                ...
                currentRowBinaryResult = new ProtonRowBinaryInputStream(
                        properties.isCompress() ? new ProtonLZ4Stream(is) : is, getConnection().getTimeZone(),
                        properties, true);
    ...
    public int executeUpdate(String sql) throws SQLException {
        Map<ProtonQueryParam, String> additionalDBParams = new EnumMap<>(ProtonQueryParam.class);
        parseSqlStatements(sql, ProtonFormat.TabSeparatedWithNamesAndTypes, additionalDBParams);
    ...
    public void addBatch(String sql) throws SQLException {
        for (ProtonSqlStatement s : ProtonSqlParser.parse(sql, null)) {
    ...
    @Override
    public ProtonConnection getConnection() throws ProtonException {
        return connection;
    }
    ...
    @Override
    public ProtonResponseSummary getResponseSummary() {
        return currentSummary;
    }

    private InputStream getLastInputStream(Map<ProtonQueryParam, String> additionalDBParams,
            List<ProtonExternalData> externalData, Map<String, String> additionalRequestParams)
    ...
    private InputStream getInputStream(ProtonSqlStatement parsedStmt,
            Map<ProtonQueryParam, String> additionalProtonDBParams, List<ProtonExternalData> externalData,
            Map<String, String> additionalRequestParams) throws ProtonException {
        ...
        additionalProtonDBParams = addQueryIdTo(additionalProtonDBParams == null
                ? new EnumMap<ProtonQueryParam, String>(ProtonQueryParam.class)
                : additionalProtonDBParams);

        URI uri = buildRequestUri(null, externalData, additionalProtonDBParams, additionalRequestParams,
                ignoreDatabase);
        ...
        if (externalData == null || externalData.isEmpty()) {
            ...
            try {
                for (ProtonExternalData externalDataItem : externalData) {
                    // proton may return 400 (bad request) when chunked encoding is used with
                    // multipart request
                    // so read content to byte array to avoid chunked encoding
                    // TODO do not read stream into memory when this issue is fixed in proton
        ...
        try {
            ...
            if (isQueryParamSet(ProtonQueryParam.SEND_PROGRESS_IN_HTTP_HEADERS, additionalProtonDBParams,
                    additionalRequestParams)) {
                Header summaryHeader = response.getFirstHeader("x-proton-summary");
                currentSummary = summaryHeader != null
                        ? JsonStreamUtils.readObject(summaryHeader.getValue(), ProtonResponseSummary.class)
                        : null;
            }
    ...
    URI buildRequestUri(String sql, List<ProtonExternalData> externalData,
            Map<ProtonQueryParam, String> additionalProtonDBParams, Map<String, String> additionalRequestParams,
            boolean ignoreDatabase) {
        try {
            List<NameValuePair> queryParams = getUrlQueryParams(sql, externalData, additionalProtonDBParams,
                    additionalRequestParams, ignoreDatabase);

            // avoid to reuse query id
            if (additionalProtonDBParams != null) {
                additionalProtonDBParams.remove(ProtonQueryParam.QUERY_ID);
            }
    ...
    private List<NameValuePair> getUrlQueryParams(String sql, List<ProtonExternalData> externalData,
            Map<ProtonQueryParam, String> additionalProtonDBParams, Map<String, String> additionalRequestParams,
            boolean ignoreDatabase) {
        ...
        Map<ProtonQueryParam, String> params = properties.buildQueryParams(true);
        String db = params.getOrDefault(ProtonQueryParam.DATABASE, currentDatabase);
        if (!ignoreDatabase && !Utils.isNullOrEmptyString(db)) {
            params.put(ProtonQueryParam.DATABASE, currentDatabase);
        }

        params.putAll(getAdditionalDBParams());

        if (additionalProtonDBParams != null && !additionalProtonDBParams.isEmpty()) {
            params.putAll(additionalProtonDBParams);
        }

        setStatementPropertiesToParams(params);

        for (Map.Entry<ProtonQueryParam, String> entry : params.entrySet()) {
    ...
    private boolean isQueryParamSet(ProtonQueryParam param,
            Map<ProtonQueryParam, String> additionalProtonDBParams,
            Map<String, String> additionalRequestParams) {
        String value = getQueryParamValue(param, additionalProtonDBParams, additionalRequestParams);

        return "true".equals(value) || "1".equals(value);
    }

    private String getQueryParamValue(ProtonQueryParam param,
            Map<ProtonQueryParam, String> additionalProtonDBParams,
            Map<String, String> additionalRequestParams) {
        ...
        if (additionalProtonDBParams != null && additionalProtonDBParams.containsKey(param)
                && !Utils.isNullOrEmptyString(additionalProtonDBParams.get(param))) {
            return additionalProtonDBParams.get(param);
        }
    ...
    private void setStatementPropertiesToParams(Map<ProtonQueryParam, String> params) {
        if (maxRows > 0) {
            params.put(ProtonQueryParam.MAX_RESULT_ROWS, String.valueOf(maxRows));
            params.put(ProtonQueryParam.RESULT_OVERFLOW_MODE, "break");
        }
        if (isQueryTimeoutSet) {
            params.put(ProtonQueryParam.MAX_EXECUTION_TIME, String.valueOf(queryTimeout));
        }
    }

    @Override
    public void sendRowBinaryStream(String sql, ProtonStreamCallback callback) throws SQLException {
        sendRowBinaryStream(sql, null, callback);
    }

    @Override
    public void sendRowBinaryStream(String sql, Map<ProtonQueryParam, String> additionalDBParams,
            ProtonStreamCallback callback) throws SQLException {
        write().withDbParams(additionalDBParams).send(sql, callback, ProtonFormat.RowBinary);
    }

    @Override
    public void sendNativeStream(String sql, ProtonStreamCallback callback) throws SQLException {
        sendNativeStream(sql, null, callback);
    }

    @Override
    public void sendNativeStream(String sql, Map<ProtonQueryParam, String> additionalDBParams,
            ProtonStreamCallback callback) throws SQLException {
        write().withDbParams(additionalDBParams).send(sql, callback, ProtonFormat.Native);
    }

    @Override
    public void sendCSVStream(InputStream content, String table, Map<ProtonQueryParam, String> additionalDBParams)
            throws SQLException {
        write().table(table).withDbParams(additionalDBParams).data(content).format(ProtonFormat.CSV).send();
    }
    ...
    @Override
    public void sendStream(InputStream content, String table, Map<ProtonQueryParam, String> additionalDBParams)
            throws SQLException {
        write().table(table).data(content).withDbParams(additionalDBParams).format(ProtonFormat.TabSeparated)
                .send();
    }

    @Deprecated
    public void sendStream(HttpEntity content, String sql) throws ProtonException {
        sendStream(content, sql, ProtonFormat.TabSeparated, null);
    }

    @Deprecated
    public void sendStream(HttpEntity content, String sql, Map<ProtonQueryParam, String> additionalDBParams)
            throws ProtonException {
        sendStream(content, sql, ProtonFormat.TabSeparated, additionalDBParams);
    }

    private void sendStream(HttpEntity content, String sql, ProtonFormat format,
            Map<ProtonQueryParam, String> additionalDBParams) throws ProtonException {
    ...
    @Override
    public void sendStreamSQL(InputStream content, String sql, Map<ProtonQueryParam, String> additionalDBParams)
            throws SQLException {
    ...
    void sendStream(Writer writer, HttpEntity content) throws ProtonException {
        HttpEntity entity = null;
        // TODO no parser involved so user can execute arbitray statement here
        try {
            String sql = writer.getSql();
            boolean isContentCompressed = writer.getCompression() != ProtonCompression.none;
            ...
            if (writer.getCompression() != ProtonCompression.none) {
            ...
            // retrieve response summary
            if (isQueryParamSet(ProtonQueryParam.SEND_PROGRESS_IN_HTTP_HEADERS, writer.getAdditionalDBParams(),
    ...
    private void checkForErrorAndThrow(HttpEntity entity, HttpResponse response)
        ...
        if (line.getStatusCode() != HttpURLConnection.HTTP_OK) {
            ...
            if (properties.isCompress()) {
                try {
                    messageStream = new ProtonLZ4Stream(new ByteArrayInputStream(bytes));
            ...
            EntityUtils.consumeQuietly(entity);
            if (bytes.length == 0) {
                throw ProtonExceptionSpecifier.specify(new IllegalStateException(line.toString()),
                        properties.getHost(), properties.getPort());
            } else {
                throw ProtonExceptionSpecifier.specify(new String(bytes, StandardCharsets.UTF_8),
                        properties.getHost(), properties.getPort());
            }
        }
    }
    ...
    private ProtonResultSet createResultSet(InputStream is, int bufferSize, String db, String table,
            boolean usesWithTotals, ProtonStatement statement, TimeZone timezone, ProtonProperties properties)
            throws IOException {
        if (isResultSetScrollable) {
            return new ProtonScrollableResultSet(is, bufferSize, db, table, usesWithTotals, statement, timezone,
                    properties);
        } else {
            return new ProtonResultSet(is, bufferSize, db, table, usesWithTotals, statement, timezone, properties);
        }
    }

    private Map<ProtonQueryParam, String> addQueryIdTo(Map<ProtonQueryParam, String> parameters) {
        if (this.queryId != null) {
            return parameters;
        }

        String queryId = parameters.get(ProtonQueryParam.QUERY_ID);
        if (queryId == null) {
            // TODO perhaps we should use TimeUUID so that it's easy to sort?
            this.queryId = UUID.randomUUID().toString();
            parameters.put(ProtonQueryParam.QUERY_ID, this.queryId);
        } else {
