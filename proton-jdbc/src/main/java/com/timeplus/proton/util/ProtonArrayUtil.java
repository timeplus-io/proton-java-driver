...
import com.timeplus.proton.ProtonUtil;
import com.timeplus.proton.response.ArrayByteFragment;
import com.timeplus.proton.response.ByteFragment;
import com.timeplus.proton.response.ProtonColumnInfo;
import com.timeplus.proton.response.parser.ProtonValueParser;

/**
 * @author Dmitry Andreev <a href="mailto:AndreevDm@yandex-team.ru"></a>
 */
public class ProtonArrayUtil {
    ...
    private ProtonArrayUtil() {
        // NOP
    }

    /**
     * @param object
     *            the object to convert to Proton-string representation
     * @param dateTimeZone
    ...
    public static Object parseArray(ByteFragment value, boolean useObjects,
        TimeZone timeZone, int arrayLevel, Class<?> elementClass, ProtonColumnInfo columnInfo) throws SQLException
    {
        if (arrayLevel > ProtonArrayUtil.MAX_ARRAY_DEPTH) {
        ...
        if (arrayLevel > 1) {
            ...
            dimensions[0] = ProtonArrayUtil.getArrayLength(trim);
            ...
            array = java.lang.reflect.Array.newInstance(
                useObjects ? elementClass : Utils.unwrap(elementClass),
                ProtonArrayUtil.getArrayLength(trim)
            );
        }
        int fieldStart = 0;
        int currentLevel = 0;
        ...
        for (int chIdx = 0; chIdx < trim.length(); chIdx++) {
            ...
            if (!inQuotation && ch == ARRAY_ELEMENTS_SEPARATOR && currentLevel == 0 || chIdx == trim.length() - 1) {
                ...
                if (arrayLevel > 1) {
                    ...
                    Object o = useObjects
                        ? ProtonValueParser.getParser(elementClass).parse(
                            fragment, columnInfo, timeZone)
                        : ProtonValueParser.getParser(elementClass).parseWithDefault(
                            fragment, columnInfo, timeZone);
    ...
    public static int getArrayLength(ByteFragment value) {
        ...
        for (int i = 0; i < value.length(); i++) {
            ...
            inQuotation = ch == ProtonArrayUtil.STRING_QUOTATION ^ inQuotation;
            if (!inQuotation) {
                if (ch == '[') {
                    ++arrayLevel;
                } else if (ch == ']') {
                    --arrayLevel;
                } else if (ch == ProtonArrayUtil.ARRAY_ELEMENTS_SEPARATOR && arrayLevel == 0) {
                    ++length;
                }
            }
        }
    ...
    public static Object parseArray(ByteFragment value,
        boolean useObjects, TimeZone timeZone, ProtonColumnInfo columnInfo)
        throws SQLException
    {
        return parseArray(value, useObjects, timeZone, columnInfo.getArrayLevel(),
            columnInfo.getEffectiveProtonDataType().getJavaClass(), columnInfo);
    }

    static Object parseArray(ByteFragment value, Class<?> clazz,
        boolean useObjects, ProtonColumnInfo columnInfo)
        throws SQLException
    {
    ...
    private static String toString(byte[] values) {
        return "'" + ProtonValueFormatter.formatBytes(values) + "'";
    }
    ...
    private static boolean needQuote(Object[] objects) {
        ...
        return objects.length == 0 || ProtonValueFormatter.needsQuoting(o);
    }
        ...
        private ArrayBuilder append(Object value) {
            ...
            if (value != null) {
                ...
                if (value instanceof String) {
                    builder.append(quote ? ProtonUtil.escape((String) value) : value);
                } else {
                    builder.append(ProtonValueFormatter.formatObject(
