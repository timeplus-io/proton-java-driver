 ...
 * Proton, but for the sake of this driver, we treat these data types as
...
public enum ProtonDataType {
    ...
    private static final Map<String, ProtonDataType> name2type;

    static {
        Map<String, ProtonDataType> map = new HashMap<>();
        String errorMsg = "[%s] is used by type [%s]";
        ProtonDataType used = null;
    ...
    ProtonDataType(JDBCType jdbcType, Class<?> javaClass,
    ...
    public static ProtonDataType fromTypeString(String typeString) {
        return name2type.getOrDefault(typeString.trim().toUpperCase(Locale.ROOT), ProtonDataType.Unknown);
    }

    public static ProtonDataType resolveDefaultArrayDataType(String typeName) {
        return name2type.getOrDefault(typeName, ProtonDataType.String);
    }
}
